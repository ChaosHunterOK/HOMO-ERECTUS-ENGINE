...
class NewStageState extends MusicBeatState
{
    ...
    var gridSizeStepper:FlxUINumericStepper;
    ...
    var assetPreviewToPath:Map<FlxSprite, String> = new Map();
    ...
    function drawGrid()
    {
        ...
        var offsetX = Std.int(FlxG.camera.scroll.x) % gridSize;
        var offsetY = Std.int(FlxG.camera.scroll.y) % gridSize;
    ...
    function addStageUI()
    {
        ...
        gridSizeStepper = new FlxUINumericStepper(10, 110, 4, 32, 4, 128, 0);
    ...
    function addAssetUI()
    {
        ...
        var loadBtn = new FlxButton(10, 20, "Import PNGs", function() {
            ...
            coolDialog.onSelectMultiple.add(function(paths:Array<String>) {
                ...
                #else
                for (path in paths) {
                    if (!epicFiles.contains(path)) epicFiles.push(path);
                }
                #end
                refreshAssetList();
            });
    ...
    {
        assetGroup.clear();
        assetPreviewToPath.clear();
        var yOff = 100;
        
        for (i in 0...epicFiles.length) {
            var path = epicFiles[i];
            #if sys
            ...
            spr.updateHitbox();
            spr.scrollFactor.set(); // Keeps previews locked inside UI panel bounds during panning
            
            assetGroup.add(spr);
            assetPreviewToPath.set(spr, path); // Statically pair preview icon directly to image path
            yOff += 60;
            #end
        }
    }
    ...
    {
        if (gridSizeStepper != null && gridSize != Std.int(gridSizeStepper.value)) {
        ...
        if (isPanning && FlxG.mouse.pressedMiddle) {
            FlxG.camera.scroll.x -= (FlxG.mouse.x - lastMousePos.x) / FlxG.camera.zoom;
            FlxG.camera.scroll.y -= (FlxG.mouse.y - lastMousePos.y) / FlxG.camera.zoom;
            lastMousePos.set(FlxG.mouse.x, FlxG.mouse.y);
            drawGrid();
        }
        ...
        if (FlxG.mouse.justPressed) {
            var clickedUI:Bool = (FlxG.mouse.x >= UI_box.x && FlxG.mouse.y >= UI_box.y && FlxG.mouse.y <= UI_box.y + UI_box.height);
            
            // Only search asset list interactions if user clicked in the UI Box domain
            if (clickedUI) {
                var clickedAsset = false;
                for (spr in assetGroup.members) {
                    if (spr != null && FlxG.mouse.overlaps(spr)) {
                        var assetPath = assetPreviewToPath.get(spr);
                        if (assetPath != null && loadedBitmaps.exists(assetPath)) {
                            // Target coordinate placement logic accounts for active zoom adjustments
                            var spawnX = (FlxG.camera.scroll.x + (FlxG.width / 2)) / FlxG.camera.zoom;
                            var spawnY = (FlxG.camera.scroll.y + (FlxG.height / 2)) / FlxG.camera.zoom;

                            var newObj = new FlxSprite(spawnX, spawnY);
                            newObj.loadGraphic(loadedBitmaps.get(assetPath));
                            spritePaths.set(newObj, assetPath);
                            newObj.updateHitbox();
                ...
                if (!clickedHandle) {
                    ...
                    if (!clickedEditor && !isPanning && FlxG.mouse.y > 50) {
                        selectedSprite = null;
                        transformMode = "none";
                        updateObjectProperties();
                        updateSelectionBox();
        ...
        if (selectedSprite != null && FlxG.mouse.pressed && !isPanning)
        {
            ...
            {
                case "move":
                    // Mouse adjustment math that handles active layer grid snapping properly relative to raw world coordinates
                    var worldMouseX = (FlxG.mouse.x + FlxG.camera.scroll.x);
                    var worldMouseY = (FlxG.mouse.y + FlxG.camera.scroll.y);
                    
                    var newX = worldMouseX - selectedSprite.width / 2;
                    var newY = worldMouseY - selectedSprite.height / 2;
                    if (snapToGrid) {
    ...
    function handleKeyboardShortcuts()
    {
        ...
        if (selectedSprite != null) {
            if (FlxG.keys.justPressed.LEFT) selectedSprite.x -= nudgeAmount;
            if (FlxG.keys.justPressed.RIGHT) selectedSprite.x += nudgeAmount;
            if (FlxG.keys.justPressed.UP) selectedSprite.y -= nudgeAmount;
            if (FlxG.keys.justPressed.DOWN) selectedSprite.y += nudgeAmount;
        ...
        if (FlxG.keys.justPressed.PAGEUP) moveLayerUp();
        if (FlxG.keys.justPressed.PAGEDOWN) moveLayerDown();
    ...
    function writeCharacters() {
        ...
        var hscriptContent = 'function onCreate() {\n';
        var id = 0;
        ...
        trace("Stage hscript exported successfully.");
        #end
    }
