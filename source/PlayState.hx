...
class PlayState extends MusicBeatState
{
	...
	var lastHealthTxtValue:Int = -1;
	var lastAccuracyTxtValue:Float = -1;
	var lastHealthTxtIsLow:Null<Bool> = null;
	...
	function sameVarsIdk(interp:Dynamic, path:String) {
		...
		interp.variables.set("oneTypeSplash", function (instant:Bool) {
			NoteSplash.oneType = instant;
		});
	...
	function makeHaxeState(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("scaleChar", function(char:String, amount:Float) {
			...
			if (target != null) {
				remove(target);
				target.setGraphicSize(Std.int(target.width * amount));
				target.updateHitbox();
				target.y *= amount;
				add(target);
			}
	...
	function makeHaxeStateUI(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("NewCustomBar", function (
			...
			function getColoredBitmap(original:BitmapData, targetColor:Int, newColor:Int):BitmapData {
				...
				if (barGraphicCache.exists(cacheKey)) return barGraphicCache.get(cacheKey);
				var newBmp = original.clone();
				for (y in 0...original.height) {
					for (x in 0...original.width) {
						var px = original.getPixel32(x, y);
						var alpha = px >> 24 & 0xFF;
						if (alpha > 0 && (px & 0x00FFFFFF) == targetColor) newBmp.setPixel32(x, y, (alpha << 24) | (newColor & 0x00FFFFFF));
					}
				}
	...
	function applyEffect(dest:String, sprite:Null<FlxSprite>, effect:ShaderEffect):Void {
	...
	override public function create()
	{
		...
		else if (FNFAssets.exists(basePath + 'assets/data/${SONG.song.toLowerCase()}/noteInfo.json'))
		...
		var assetPath = basePath + 'assets/'; // Cache base path

		function resolveDialog(path:String, baseName:String):Null<String> {
			var withSuffix = assetPath + path + baseName + dialogSuffix + ".txt";
			if (FNFAssets.exists(withSuffix))
				return withSuffix;
			var normal = assetPath + path + baseName + ".txt";
		...
		if (useSongBar) {
			// todo, add options
			var timeBarPath = basePath + 'assets/images/custom_ui/ui_packs/' + uiSmelly.uses + 'timeBar.png';
			var altTimeBarPath = basePath + 'assets/images/timeBar.png';
			var defaultPath = basePath + 'assets/images/healthBar.png';
			var timeBarGraphic = FNFAssets.getBitmapData(
				FNFAssets.exists(timeBarPath) ? timeBarPath :
				FNFAssets.exists(altTimeBarPath) ? altTimeBarPath : defaultPath
			);
			songPosBG = new FlxSprite(0, 10).loadGraphic(timeBarGraphic);
			if (downscroll)
		...
		var healthBarPath = basePath + 'assets/images/custom_ui/ui_packs/' + uiSmelly.uses + 'healthBar.png';
		var healthBarGraphic = FNFAssets.getBitmapData(
			FNFAssets.exists(healthBarPath) ? healthBarPath : basePath + 'assets/images/healthBar.png'
		);
		healthBarBG = new FlxSprite(0, FlxG.height * 0.9).loadGraphic(healthBarGraphic);
		if (downscroll)
			healthBarBG.y = 50;
		...
		var leftSideFill = duoMode ? dad.opponentColor : (opponentPlayer ? dad.opponentColor : dad.enemyColor);
		var rightSideFill = duoMode ? boyfriend.bfColor : (opponentPlayer ? boyfriend.bfColor : boyfriend.playerColor);
		healthBar.createFilledBar(leftSideFill, rightSideFill);
		// healthBar
		add(healthBar);

		var fontPath = basePath + "assets/fonts/vcr.ttf";
		scoreTxt = new FlxText(healthBarBG.x, healthBarBG.y + 40, 0, "", 200);
		scoreTxt.setFormat(fontPath, 20, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();

		healthTxt = new FlxText(healthBarBG.x + healthBarBG.width - 300, scoreTxt.y, 0, "", 200);
		healthTxt.setFormat(fontPath, 20, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		healthTxt.scrollFactor.set();
		healthTxt.visible = false;
		accuracyTxt = new FlxText(healthBarBG.x, scoreTxt.y, 0, "", 200);
		accuracyTxt.setFormat(fontPath, 20, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		accuracyTxt.scrollFactor.set();
		// shitty work around but okay
		accuracyTxt.visible = false;
		difficTxt = new FlxText(10, FlxG.height, 0, "", 150);
		
		difficTxt.setFormat(fontPath, 15, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		...
		var stageJson:Dynamic = CoolUtil.parseJson(FNFAssets.getText(basePath + "assets/images/custom_stages/custom_stages.json"));
	...
	private function generateSong(dataPath:String):Void
	{
		...
		Conductor.changeTimeSig(
			(songData.timeSigNumerator != null && songData.timeSigNumerator > 0) ? songData.timeSigNumerator : 4,
			(songData.timeSigDenominator != null && songData.timeSigDenominator > 0) ? songData.timeSigDenominator : 4
		);
		lastTimeSigSection = -1;
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (rawNoteData % (ammo * 2) > ammo - 1) gottaHitNote = !mustHit;
				...
				if (susLength != 0) {
					...
					for (susNote in 0...(floorSus + 2))
					{
						...
						else if (susLength > susNote)
						{
							...
							sustainNote.dontCountNote = swagNote.dontCountNote || sustainNote.dontCountNote;
							sustainNote.dontMiss = swagNote.dontMiss || sustainNote.dontMiss;
	...
	function comboBreak(dir:Int, playerOne:Bool = true, rating:String = 'miss') {
		...
		var coolor = switch (rating) {
			case 'miss': missBreakColor;
			case 'wayoff': wayoffBreakColor;
			case 'shit': shitBreakColor;
			default: return;
		}
	...
	function beatSteps():Int
	{
		var spb:Dynamic = Conductor.stepsPerBeat;
		if (spb == null || spb <= 0)
			spb = 4;
		return Std.int(spb);
	}
	function tweenCamIn():Void
		addTrackedTween(FlxG.camera, {zoom: 1.3}, (Conductor.stepCrochet * beatSteps() / 1000), {ease: FlxEase.elasticInOut});
	...
	override function openSubState(SubState:FlxSubState)
	{
		...
		for (tween in modTweens) tween.active = false;
		for (timer in modTimers) timer.active = false;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (duoMode) controls.setKeyboardScheme(Duo(true));
			else if (!opponentPlayer) controls.setKeyboardScheme(Solo(false));
			...
			{
				DiscordClient.changePresence(customPrecence, SONG.song + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), iconRPC, playingAsRpc);
			}
			#end
		}

		for (tween in modTweens) tween.active = true;
		for (timer in modTimers) timer.active = true;
		super.closeSubState();
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (++currentFrames >= FlxG.save.data.fpsCap) {
			...
			for (readIdx in 0...notesHitArray.length) {
				var cock = notesHitArray[readIdx];
				if (cock != null && cock.getTime() >= cutoff) {
					notesHitArray[writeIdx] = cock;
					writeIdx++;
				}
			}
			notesHitArray.resize(writeIdx);
		...
		var section = PlayState.SONG.notes[Conductor.getSectionAtStep(curStep)];
		if (endingSong)
			return;
		if (section != null)
		{
			...
			inline function updateCamFollow(char:Character, defaultX:Float, defaultY:Float)
			{
				if (char.isCustom)
					...
					camFollow.setPosition(char.getMidpoint().x + defaultX,char.getMidpoint().y + defaultY);
			}
			if (!mustHit)
			{
		...
		if (generatedMusic)
		{
			...
			if ((!duoMode && !opponentPlayer) || demoMode) {
				for (spr in enemyStrums.members) {
					if (spr != null && strumming2[spr.ID])
	...
	override function beatHit()
	{
		...
		var beatHitSection = Conductor.getSectionAtStep(curStep);
		if (SONG.notes[beatHitSection] != null)
		{
			if (SONG.notes[beatHitSection].changeBPM)
			{
				Conductor.changeBPM(SONG.notes[beatHitSection].bpm);
				FlxG.log.add('CHANGED BPM!');
			}
			// else
			// Conductor.changeBPM(SONG.bpm);

			if (Reflect.field(SONG.notes[beatHitSection], "changeTimeSig") == true && beatHitSection != lastTimeSigSection)
			{
				var secTimeSigNum:Dynamic = Reflect.field(SONG.notes[beatHitSection], "timeSigNum");
				var secTimeSigDen:Dynamic = Reflect.field(SONG.notes[beatHitSection], "timeSigDen");
				if (secTimeSigNum != null && secTimeSigNum > 0 && secTimeSigDen != null && secTimeSigDen > 0)
					Conductor.changeTimeSig(secTimeSigNum, secTimeSigDen);
			}
